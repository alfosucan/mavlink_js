...
class MavLink {
    ...
    clearCommandQ() {
        ...
        for (var i = 0; i < command_Q.length; i++){
            ...
            if ( span > 1000) {
                log_to_console(`Removing entry ${command_Q[i].command} of ${span} ms old`);
    ...
    sendCommand(command, ...params) {
        ...
        let index = command_Q.findIndex(val => val.command == cmd_id);
        if (index > -1) {
            log_to_console(`Message ${command} already sent`)
            return;
        }
        let now = Date.now();
        command_Q.push({name: command, command: cmd_id, time: now});
        if (typeof params[0] == "string")
        params[0] = mavID[params[0]]
        var command_msg = new this.mavlink_.messages.command_long(
            2,
            1,
            cmd_id,
            101,
    ...
    commandACKCallback(ack_msg) {
        ...
        if (index > -1) {
            log_to_console(`Removed ${command_Q[index].name} ID[${ack_msg.command}] at position ${index} with result ${ack_msg.result}`);
        ...
        switch (ack_msg.result) {
            case mavID['MAV_RESULT_TEMPORARILY_REJECTED']:
                console.log(`ERROR: MAV_RESULT_TEMPORARILY_REJECTED ${cmd_name} ID:[${id}]`);
                break;
    ...
    requestMessage(mav_msg) {
        this.sendCommand('MAV_CMD_REQUEST_MESSAGE',
            mavID[mav_msg], 0,0,0,0,0,1)
    }
